...
# MAGIC
# MAGIC Unlike the previous notebook-tasks this notebook isn't doing the task so much as it is confirming a task is done. Specifically this notebook is going to verify that the following inner-loop[...]
# MAGIC
# MAGIC   * **<a href="$./agent_model/02_01_Candidate_Runs" target="_blank">02_01_Candidate_Runs</a>**: This will make sure that the candidate experiment model runs that are listed in the candidate[...]
...
# MAGIC   * **<a href="$./agent_models/02_03_Review_App_Feedback" target="_blank">02_03_Review_App_Feedback</a>**: Inspect feedback to detemine potential updates to parameters, scoring, or champio[...]
...
# MAGIC
# MAGIC **NOTE**: Be aware that agent model comparison happens twice! The first is to select the best candidate model. The second is to determine if the best candidate is good enough to unseat the[...]
# MAGIC
# MAGIC The above inner-loop tasks are consistant with our governance diagram. However, now that we'll be interacting with our agent experiment, we can also show a graphical representation of the [...]
# COMMAND ----------
...
# MAGIC
# MAGIC **Note**: There are ways to set a standard that a set number of review entries are created and have populated feedback fields, but that level of rigor isn't applied in this solution accele[...]
# COMMAND ----------
import requests
...
# MAGIC
# MAGIC Now we are at the point where we have all assets in the proper state. All code for these notebooks will be in a state that the model can be replicated. Thus, if the all complete checks ar[...]
# COMMAND ----------
