...
# MAGIC
# MAGIC We already have run evaluate and gotten metrics for each of our models. It's now time to look at those results, protentially add further metrics and determine the best. In this notebook, we[...]
# MAGIC
# MAGIC **NOTE**: It is also possible that only a single model is created for a release. This step should still be taken as this will not always be the case and results written back to the experime[...]
# MAGIC
# MAGIC It's always possible to deploy multiple models for feedback, but this will complicate the task of getting feedback equally across models. Instead, our convention will be to select a single[...]
...
# MAGIC  - Metrics
# MAGIC
# MAGIC **NOTE**: There is an investment bias towards when selecting a model that you have spent the most time on. Thus it is important that the selection criteria are understood ahead of actual s[...]
# MAGIC
# MAGIC **NOTE**: All criteria must be in a model run for best model selection. Meaning that you should make use of the tags and metrics feature in MLFlow so that all attributes of a decision are [...]
# MAGIC
...
# MAGIC
# MAGIC [Deploying an Agent for Generative AI Applications](https://docs.databricks.com/en/generative-ai/deploy-agent.html#deploy-an-agent-for-generative-ai-application) is still private preview. [...]
# MAGIC
# MAGIC  * We are using LLM as a judge that requires a deployment of a model to evaluate our model
# MAGIC  * Our metrics are not simple scalers, instead we have a lot of complex type data to inspect. Infact, there are 3 new [agent-enhanced-inference-tables](https://docs.databricks.com/en/gener[...]
# MAGIC
...
Your inputs are invaluable for the development team. By providing detailed feedback and corrections, you help improve the chatbot's accuracy and usefulness for biomedical research queries.

- Please test a variety of realistic biomedical questions.
- Note any errors, omissions, or hallucinations in the chatbot's responses.
- Provide corrections or suggested improvements where appropriate.
- Rate the responses for relevance, correctness, and groundedness.
- If the chatbot's answer is incomplete or overly generic, please indicate so.

Thank you for your help in developing a high-quality biomedical research assistant!
"""
