...
# MAGIC
# MAGIC It is both possible and likely that in this process there will be instances where there will be feedback that will change the critiera of best model selection. This is ok, but you don't want to continually change criteria as feedback is reviewed.
# MAGIC
...
# MAGIC
# MAGIC **NOTE**: It is not appropriate to introduce new dependencies during the inner-loop. It is also not appropriate to create new eval_ds entries. Instead, annotate these opportunities. If within the normal cycle, you need to create a new eval_ds, that should be done ahead of time.
# MAGIC
